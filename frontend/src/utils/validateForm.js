...
export default function validate({
  ...
  (charzData.search_types || []).forEach(searchType => {
    ['CREST', 'BIST', 'PBIST'].forEach(testType => {
      ...
      if (!table.tp || table.tp.trim() === '') newErrors[`charz_${searchType}_${testType}_tp`] = 'Test Points are required for ' + searchType + ' / ' + testType;
      if (!table.search_start || isNaN(Number(table.search_start))) newErrors[`charz_${searchType}_${testType}_search_start`] = 'Search Start is required and must be a number for ' + searchType + ' / ' + testType;
      if (!table.search_end || isNaN(Number(table.search_end))) newErrors[`charz_${searchType}_${testType}_search_end`] = 'Search End is required and must be a number for ' + searchType + ' / ' + testType;
      if (!table.resolution || isNaN(Number(table.resolution))) newErrors[`charz_${searchType}_${testType}_resolution`] = 'Resolution is required and must be a number for ' + searchType + ' / ' + testType;
      if (!table.search_step || isNaN(Number(table.search_step))) newErrors[`charz_${searchType}_${testType}_search_step`] = 'Search Step is required and must be a number for ' + searchType + ' / ' + testType;
    });
  });
