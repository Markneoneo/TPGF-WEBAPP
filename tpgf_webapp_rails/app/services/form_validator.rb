class FormValidator
  ...
  def validate_production_mappings
    ...
    @params[:show_production_for_core].to_h.each do |idx, enabled|
      ...
      if flow_orders.empty?
        @errors["flow_orders_core_#{idx}"] = "At least one flow order must be selected for Core Type #{idx.to_i + 1}"
        next
      end
  ...
  def validate_production_mapping(mapping, order, core_idx)
   ...
   # Validate spec variable (unless use_power_supply is checked)
   if mapping[:use_power_supply] != "on" && mapping[:spec_variable].blank?
     @errors["spec_variable_#{order}_core_#{core_idx}"] = "#{order}: Spec variable is required"
   end
   
   # Validate frequency
   if mapping[:frequency].blank?
     @errors["frequency_#{order}_core_#{core_idx}"] = "#{order}: Frequency is required"
   elsif !mapping[:frequency].match?(/^\d+$/)
     @errors["frequency_#{order}_core_#{core_idx}"] = "#{order}: Frequency must be a number"
   end
   
   # Validate register size
   if mapping[:register_size].blank?
     @errors["register_size_#{order}_core_#{core_idx}"] = "#{order}: Register size is required"
   elsif !mapping[:register_size].match?(/^\d+$/)
     @errors["register_size_#{order}_core_#{core_idx}"] = "#{order}: Register size must be a number"
   end
   ...
   if mapping[:insertion].present?
     ...
     if insertions.any?(&:blank?)
       @errors["insertion_#{order}_core_#{core_idx}"] = "#{order}: Insertion list has empty values"
     end
   end
 end
     ...
     if charz_data[:search_types].present?
       charz_data[:search_types].each do |search_type|
         ...
         selected_test_types.each do |test_type|
           ...
           %w[tp search_start search_end search_step resolution].each do |field|
